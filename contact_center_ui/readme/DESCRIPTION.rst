...
CRM extensions and origin cards
===============================

The UI depends on ``contact_center_base`` and native ``web``. Customer documents
are supplied by the optional ``contact_center_crm`` extension; service cases and
CRM stage actions belong to ``contact_center_kanban``. Neither opening a conversation
nor displaying an origin card creates a lead or links a commercial record.

The handshake panel lists records of the linked contact and commercial customer.
That customer view is different from an explicit conversation-to-opportunity link.
