...
class CrmLead(models.Model):
    ...
    def _contact_center_lock_conversation_graph(
        ...
        """Fence optional pipeline authorities before the conversation graph."""

        leads = self
        if not stage_sync_graph_is_locked(self.env):
            leads = self._contact_center_lock_link_graph(
                touch_leads=touch_leads, touch_cases=touch_channels
            )
        return super(CrmLead, leads)._contact_center_lock_conversation_graph(
